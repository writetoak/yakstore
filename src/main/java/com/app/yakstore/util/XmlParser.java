...
public class XmlParser
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(Herd.class, LabYak.class);
		javax.xml.bind.Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		Herd herd = (Herd) jaxbUnmarshaller.unmarshal(new File(fileName));
		...
		*/
		...
		int noOfSkins = 0;
		float dayToBeAddedInAge = nthDay;

		for(Yak yak:herd.getYaks())
		{

			ageOnNthDay = yak.getAgeInYears() + (dayToBeAddedInAge/100);
